        ...
        {rows.map((c) => {
          ...
          return (
            <li key={c.id}>
              <Link
                href={`/captures/${c.id}`}
                className="flex flex-col gap-1.5 rounded-xl border border-zinc-200 bg-white px-4 py-3 text-sm transition hover:border-zinc-300 hover:bg-zinc-100/60 dark:border-zinc-900 dark:bg-zinc-950/40 dark:hover:border-zinc-800 dark:hover:bg-zinc-900/40"
              >
                <div className="flex items-center gap-2 text-xs">
                  ...
                  <span className="ml-auto flex items-center gap-1.5">
                    ...
                    {taskCount === 0 && noteCount === 0 && (
                      <span className="text-zinc-400 dark:text-zinc-600">no items</span>
                    )}
                  </span>
                </div>
                <p className="line-clamp-2 text-zinc-800 dark:text-zinc-200">{c.raw_text}</p>
              </Link>
            </li>
          );
        })}
